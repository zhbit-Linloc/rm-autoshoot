...
void armorFind::Init(Mat inputImg,Mat outputImg, int color, int Contours){
    this->inputImg = inputImg;
    this->outputImg = outputImg;
    SetColor(color);
    ContourThread(Contours);
    armorFindInit();
}
...
//    cout << "offset_roi_point" << offset_roi_point << endl;
    ...
    if(found_flag)
    {
#ifndef SHOW_DRAW_SPOT
        target.draw_spot2(img, offset_roi_point);
#endif
        Point2f point_tmp[4];
        Point2f point_2d[4];
        // 左右灯条分类，本别提取装甲板四个外角点
